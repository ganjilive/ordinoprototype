...
import { useTestResultsWorkflow } from '../../hooks/useTestResultsWorkflow';
import { UpdateDashboards } from './steps/execution';
import { NotificationToast } from './steps';
import { CollectResults, AnalyzeResults, IntelligentDashboards } from './steps/results';

const stepDescriptions = [
  '',
  'Ordino collects test results from local execution, CI/CD pipelines, and integration test suites.',
  'Ordino performs comprehensive analysis: pass/fail metrics, failure classification, and trend patterns.',
  'Ordino generates intelligent role-based dashboards customized for QA Leads, Managers, and Developers.',
  'Ordino distributes reports to stakeholders, triggers alerts for critical issues, and creates Jira tickets for product defects.',
];

// Time metrics for Test Results workflow (in minutes for manual, seconds for Ordino)
const testResultsTimeMetrics = [
  { manual: 0, ordino: 0 }, // Step 0 (not started)
  { manual: 20, ordino: 15 }, // Step 1: Collect results (20 min vs 15 sec)
  { manual: 60, ordino: 30 }, // Step 2: Analyze results (1 hr vs 30 sec)
  { manual: 45, ordino: 20 }, // Step 3: Generate reports (45 min vs 20 sec)
  { manual: 15, ordino: 10 }, // Step 4: Distribute (15 min vs 10 sec)
];

export function TestResultsWorkflowComponent() {
  const {
    currentStep,
    steps,
    isPlaying,
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-ordino-text">Test Results Generation</h2>
            <p className="text-sm text-ordino-text-muted">
              Experience Ordino's 4-step automated test results workflow
            </p>
          </div>
              ...
              {isComplete && (
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <p className="text-ordino-text-muted">
                      Results collected, analyzed with comprehensive metrics, intelligent dashboards generated,
                      and stakeholders notified — automatically.
                    </p>
                  </div>
      ...
      {safeCurrentStep === 0 && (
        <Card className="text-center py-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h3 className="text-2xl font-bold text-ordino-text mb-2">
              Ready for Intelligent Test Results?
            </h3>
            <p className="text-ordino-text-muted max-w-md mx-auto mb-6">
              Watch Ordino collect results from multiple sources, perform comprehensive analysis with trend detection,
              generate role-based dashboards, and distribute actionable insights — all automatically.
            </p>
