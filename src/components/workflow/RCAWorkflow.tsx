...
import { useRCAWorkflow } from '../../hooks/useRCAWorkflow';
import { FailurePatternDetection, AutomatedRCAAnalysis, RootCauseIdentified, RCAApproval } from './steps/rca';

const stepDescriptions = [
  '',
  'Ordino detects that test case TC-AUTH-045 has been failing consistently in the last 5 releases.',
  'Ordino performs automated investigation: analyzing test logs, code changes, and environment configuration.',
  'Root cause identified: outdated element locator in test script caused by recent UI refactoring.',
  'RCA findings sent to QA team and developers for review, approval, and corrective actions.',
];

// Time metrics for RCA workflow (in minutes for manual, seconds for Ordino)
const rcaTimeMetrics = [
  { manual: 0, ordino: 0 }, // Step 0 (not started)
  { manual: 30, ordino: 15 }, // Step 1: Detect patterns (30 min vs 15 sec)
  { manual: 180, ordino: 120 }, // Step 2: Investigate (3 hours vs 2 min)
  { manual: 60, ordino: 30 }, // Step 3: Root cause identification (1 hour vs 30 sec)
  { manual: 30, ordino: 20 }, // Step 4: Review & approval (30 min vs 20 sec)
];

export function RCAWorkflowComponent() {
  const {
    currentStep,
    steps,
    isPlaying,
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 4:
        return <RCAApproval onApprove={approveFindings} />;
      default:
        return null;
    }
  };

  // Step 4 (Review & Approve) blocks until findingsApproved
  const isBlockingStep = currentStep === 4 && !findingsApproved;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-ordino-text">Root Cause Analysis (RCA)</h2>
            <p className="text-sm text-ordino-text-muted">
              Experience Ordino's 4-step automated root cause analysis workflow
            </p>
          </div>
              ...
              {isComplete && (
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <p className="text-ordino-text-muted">
                      Failure pattern detected, investigation performed, root cause identified, and team notified — automatically.
                    </p>
                  </div>
        ...
        <Card className="text-center py-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-ordino-text-muted max-w-md mx-auto mb-6">
              Watch Ordino detect failure patterns in your test suite, perform deep automated investigation,
              identify root causes, and coordinate team review — all automatically.
            </p>
