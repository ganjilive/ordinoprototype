...
import type { FailureStepStatus } from '../../hooks/useTestFailureDemo';

interface FailureTimelineProps {
  steps: { id: number; status: FailureStepStatus }[];
  currentStep: number;
}

const stepTitles = [
  'Test Failures Detected',
  'Ordino Investigation',
  'Bug Detected',
  'JIRA Bug Report',
...
export function FailureTimeline({ steps, currentStep }: FailureTimelineProps) {
  return (
    <div className="w-64 flex-shrink-0">
      <h3 className="text-sm font-semibold text-ordino-text-muted mb-4 uppercase tracking-wider">
        Failure Investigation Progress
      </h3>
