...
import { TestDesignTimeline } from './TestDesignTimeline';
import { useTestDesignWorkflow } from '../../hooks/useTestDesignWorkflow';
import { TestArtifactLookup } from './steps/TestArtifactLookup';
import { TestDesignDrafting } from './steps/TestDesignDrafting';
import { TestDesignReview } from './steps/TestDesignReview';
import { NotificationToast } from './steps/NotificationToast';

const stepDescriptions = [
  '',
  'Ordino collects test plan and requirements with acceptance criteria from Jira.',
  'Ordino creates hierarchical test design showing all test paths and scenarios.',
  'Test design submitted to QA Lead for review and approval.',
  'QA team notified of approved test design via Slack and email.',
];

// Time metrics for Test Design workflow (in minutes for manual, seconds for Ordino)
const testDesignTimeMetrics = [
  { manual: 0, ordino: 0 }, // Step 0 (not started)
  { manual: 15, ordino: 8 }, // Step 1: Collect information
  { manual: 90, ordino: 45 }, // Step 2: Test design creation
  { manual: 20, ordino: 5 }, // Step 3: Approval
  { manual: 5, ordino: 2 }, // Step 4: Notification
];

export function TestDesignWorkflowComponent() {
  const {
    currentStep,
    steps,
    isPlaying,
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 3:
        return <TestDesignReview onApprove={approveDesign} />;
      case 4:
  ...
  // Step 3 (Approval) blocks until designApproved
  const isBlockingStep = currentStep === 3 && !designApproved;

  // Calculate cumulative metrics
  const cumulativeManualMinutes = testDesignTimeMetrics
    .slice(0, currentStep + 1)
    .reduce((acc, curr) => acc + curr.manual, 0);

  const cumulativeOrdinoSeconds = testDesignTimeMetrics
    .slice(0, currentStep + 1)
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-ordino-text">Test Design</h2>
            <p className="text-sm text-ordino-text-muted">
              Experience Ordino's 4-step test design workflow
            </p>
          </div>
          ...
          <Card className="bg-ordino-primary/10 border-ordino-primary/30">
            <div className="flex items-center gap-3 p-2">
              ...
              <div>
                ...
                <p className="text-xl font-mono text-ordino-primary font-bold">{cumulativeOrdinoSeconds} sec</p>
              </div>
            </div>
          </Card>
      ...
      {currentStep > 0 && (
        <div className="flex gap-6">
          {/* Timeline sidebar */}
          <TestDesignTimeline steps={steps} currentStep={currentStep} />
              ...
              {isComplete && (
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <p className="text-ordino-text-muted">
                      Test design created, approved, and stakeholders notified — automatically.
                    </p>
                  </div>
      ...
      {currentStep === 0 && (
        <Card className="text-center py-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h3 className="text-2xl font-bold text-ordino-text mb-2">
              Ready for AI-Powered Test Design?
            </h3>
            <p className="text-ordino-text-muted max-w-md mx-auto mb-6">
              Watch Ordino collect requirements and test plans, create a comprehensive hierarchical
              test design, route it for QA Lead approval, and notify the team — automatically.
            </p>
